...
from .cocob_opt import COCOBBackprop
...
class BaseModel(torch.nn.Module):
    ...
    def compute_auc_tar_far(
        ...
        TARs, FARs, thresholds = self.compute_tar_far(
...
class GNN(BaseModel):
    def __init__(
        ...
        self.use_cocob_optim = use_cocob_optim
        self.s = s
        self.m = m
    ...
    def init_optimizer(self):
        if self.use_cocob_optim:
            print("Using COCOBBackprop")
            self.opt = COCOBBackprop(self.model.parameters())
        else:
