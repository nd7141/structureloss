...
def get_train_graphs_and_labels(folder, datapart="train"):
    ...
    labels = [label2ix[label] for label in labels]
...
def scale_features(graphs, test_graphs1, test_graphs2):
    loader = GraphDataLoader(
        graphs, batch_size=len(graphs), drop_last=False, shuffle=False
    )
    for gs in loader:
    ...
    for g, g1, g2 in zip(graphs, test_graphs1, test_graphs2):
        g.ndata["node_attr"] = torch.tensor(
            min_max_scaler.transform(g.ndata["node_attr"])
        ).float()
        g1.ndata["node_attr"] = torch.tensor(
            min_max_scaler.transform(g1.ndata["node_attr"])
        ).float()
        g2.ndata["node_attr"] = torch.tensor(
            min_max_scaler.transform(g2.ndata["node_attr"])
        ).float()


def load_data(folder):
    graphs, labels = get_train_graphs_and_labels(folder, datapart="train")
    test_graphs1, test_graphs2, test_labels = get_test_graphs_and_labels(
        folder, datapart="test"
    )
...
def create_configs(base_config, config_folder):
    ...
    for arcface in [True, False]:
        for gnn_name in ["gcn", "gat", "agnn"]:
            ...
            config["use_cocob_optim"] = base_config["use_cocob_optim"]
...
def write_plots(gnn_model, metrics, plots_exp_folder):
    gnn_model.plot_interactive(
        [metrics],
        legend=["Train", "Test"],
        title="TAR_FAR AUC",
        metric_name="tar_auc",
        start_from=0,
        output_fn=plots_exp_folder + "/tar_far.png",
        to_show=False,
    )
    gnn_model.plot_interactive(
        [metrics],
        ...
        metric_name="auc",
        start_from=0,
        output_fn=plots_exp_folder + "/roc.png",
        to_show=False,
    )
    gnn_model.plot_interactive(
        [metrics],
        ...
        metric_name="acc",
        start_from=0,
        output_fn=plots_exp_folder + "/acc.png",
        to_show=False,
    )
    gnn_model.plot_interactive(
        [metrics],
        ...
        metric_name="loss",
        start_from=0,
        output_fn=plots_exp_folder + "/loss.png",
        to_show=False,
    )
...
if __name__ == "__main__":
    ...
    results_fn = results_folder + "/results.txt"
    ...
    for config_fn in config_fns:
        ...
        gnn_model = GNN(
            ...
            residual=False,
            use_cocob_optim=config["use_cocob_optim"],
            s=config["s"],
            m=config["m"],
        )

        metrics = gnn_model.fit(
            graphs,
            labels,
            test_graphs1,
            test_graphs2,
